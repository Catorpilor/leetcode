...
func TestWithBunchCommands(t *testing.T) {
	...
	obj.Put(2, 2)
	if obj.Get(2) != 2 {
		t.Fatalf("after put (2,2), get should return 2 but got:%d", obj.Get(2))
	}
	obj.Put(3, 3)
	if obj.Get(3) != 3 {
		t.Fatalf("after put (3,3), get should return 3 but got:%d", obj.Get(3))
	}
	if obj.Get(1) != 1 {
		t.Fatalf("after put (1,1), get should return 1 but got:%d", obj.Get(1))
	}
	...
	if obj.Get(2) != -1 {
		t.Fatalf("after put (4,4), get should return -1 but got:%d", obj.Get(2))
	}
	t.Log("pass")
}
