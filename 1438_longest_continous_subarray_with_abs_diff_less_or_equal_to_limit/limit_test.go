...
func TestLongestSubarrayUseDequev2(t *testing.T) {
    st := []struct {
        name  string
        nums  []int
        limit int
    ...
    for _, tt := range st {
        t.Run(tt.name, func(t *testing.T) {
            out := useDequev2(tt.nums, tt.limit)
            if out != tt.exp {
                t.Fatalf("with input nums:%v and limit:%d wanted %d but got %d", tt.nums, tt.limit, tt.exp, out)
            }
            t.Log("pass")
...
func TestLongestSubarrayUseSlice(t *testing.T) {
    st := []struct {
        name  string
        nums  []int
        limit int
    ...
    for _, tt := range st {
        t.Run(tt.name, func(t *testing.T) {
            out := useSliceAsDeque(tt.nums, tt.limit)
            if out != tt.exp {
                t.Fatalf("with input nums:%v and limit:%d wanted %d but got %d", tt.nums, tt.limit, tt.exp, out)
            }
            t.Log("pass")
